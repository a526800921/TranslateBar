...
- `docs/PLAN_MAP.md` 更新当前状态和本计划证据链接。
...
- 与本计划相关的覆盖包括 `ModelsTests.swift`、`TranslationServiceTests.swift`、`TranslatePanelViewTests.swift`、`AppDelegateTests.swift` 和 `URLSessionProtocolTests.swift`。
- 覆盖面包括请求/响应 Codable、翻译模式和中英文判定、非流式翻译、错误映射、取消状态、菜单栏启动结构、面板基础渲染和 URLSession 协议适配。
- 计划完成时已有 Debug/Release 构建和手动验收证据；当前达标验证以 XCTest 测试通过、构建通过和治理检查通过为准。
